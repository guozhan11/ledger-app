...
- Project the savings goal on either Spending or Spending + Income, so you decide whether the forecast assumes your income keeps arriving — the choice is saved and only affects the projection
- Record positive income, which is subtracted from consumption, and exclude positive or negative one-time adjustments from spending trends without changing the real account total
